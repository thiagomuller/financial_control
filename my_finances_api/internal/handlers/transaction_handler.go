...
func (h *Handler) createTransaction(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.QueryRowContext(r.Context(),
		`INSERT INTO transactions (user_id, bank_account_id, name, value, operation, date)
		 VALUES ($1,$2,$3,$4,$5,$6)
		 RETURNING id, user_id, bank_account_id, name, value, operation, date, created_at, updated_at`,
...
func (h *Handler) updateTransaction(w http.ResponseWriter, r *http.Request) {
	...
	// Load old transaction to reverse its balance effect
	var oldValue float64
	var oldOp, oldAccountID string
	err = tx.QueryRowContext(r.Context(),
		`SELECT value, operation, bank_account_id FROM transactions WHERE id=$1 AND user_id=$2`,
		id, userID,
	).Scan(&oldValue, &oldOp, &oldAccountID)
	if err == sql.ErrNoRows {
